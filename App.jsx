...
async function getInsights(ctx){
  var prompt = "You are a senior commercial strategy advisor. You speak plainly \u2014 no jargon, no frameworks, no consulting-speak. You explain things the way a smart operator would explain them to a peer over coffee.\n\nA " + ctx.role + " at a " + ctx.size + " " + ctx.vertLabel + " company just completed a promise-to-proof diagnostic.\n\nScores (each out of 20):\n  Promise\u2192Deliver: " + ctx.bs[0] + "\n  Deliver\u2192Operate: " + ctx.bs[1] + "\n  Operate\u2192Measure: " + ctx.bs[2] + "\n  Measure\u2192Prove: " + ctx.bs[3] + "\n  Prove\u2192Decide: " + ctx.bs[4] + "\n  Total: " + ctx.total + "/100\n\nCurrent NRR: " + ctx.nrr + "\n\nSourced benchmarks (use these, don't invent others):\n- Companies with strong outcome practices achieve NRR 7-16 points higher than peers (McKinsey)\n- Top-quartile B2B SaaS NRR is 113% (Benchmarkit 2025)\n- Companies above 120% NRR get valuation multiples of 21x vs 9x (SaaS Capital)\n\nHere is what you need to know about " + ctx.vertLabel + " companies:\n\nValue promise: " + ctx.vd.valuePromise + "\nTypical outcomes: " + ctx.vd.typicalOutcomes + "\n\nWhy proof is specifically hard in this vertical: " + ctx.vd.whyProofIsHard + "\n\nThe shape of the proof problem: " + ctx.vd.proofShape + "\n\nWhich breaks hurt most: " + ctx.vd.whatMakesItWorse + "\n\nWhat approaches don't work here (avoid suggesting these): " + ctx.vd.whatDoesntWork + "\n\nEvidence that actually drives renewals: " + ctx.vd.renewalEvidence + "\n\nIMPORTANT INSTRUCTIONS:\n- Write like an experienced operator talking to a peer. No consulting frameworks. No buzzwords.\n- Be specific to " + ctx.vertLabel + ". If your advice would work for any SaaS company, it's too generic \u2014 rewrite it.\n- Check your suggestions against the 'what doesn't work' list above. If your suggestion appears there, choose a different one.\n- Do NOT give a step-by-step implementation plan. Instead, identify where the architecture is broken and what areas are worth investigating.\n- Do NOT be salesy. Be honest and direct.\n- Any financial estimate beyond the sourced benchmarks should be labeled as 'directional' not presented as fact.\n\nRespond ONLY with valid JSON. No backticks, no markdown.\n\n{\n  \"nrrGapAnalysis\": \"3-4 sentences. How is the proof gap specifically costing this " + ctx.vertLabel + " company NRR? Name the mechanism \u2014 not just 'can't prove value' but HOW the inability to prove value in this specific vertical leads to lost revenue (discount pressure, stalled expansion, churn, etc). Reference McKinsey/Benchmarkit/SaaS Capital by name. Any estimate beyond the sourced data should say 'directionally' or 'likely'.\",\n  \"verticalDiagnosis\": \"4-5 sentences. What do these scores tell you about this " + ctx.vertLabel + " company? Explain in plain language why proof is structurally hard in this space \u2014 where does the evidence actually live, why can't the platform capture it, and what happens at renewal because of that gap? Name specific systems (ERP, HRIS, CRM, etc) and specific conversations that go wrong. This MUST be specific to " + ctx.vertLabel + " \u2014 if you could swap in a different vertical and the text still makes sense, it's too generic.\",\n  \"biggestBreakImpact\": \"3-4 sentences. Their weakest link is " + BREAKS[ctx.weak].name + " (" + ctx.bs[ctx.weak] + "/20). What does this break specifically cost a " + ctx.vertLabel + " company at " + ctx.size + "? Name the revenue mechanism. If this isn't actually the most dangerous break for this vertical (check the 'which breaks hurt most' section above), say so \u2014 acknowledge their weak point but flag the structural one too.\",\n  \"areasToInvestigate\": [\n    { \"area\": \"short label for the area\", \"observation\": \"2-3 sentences. Describe what's likely broken and why it matters for " + ctx.vertLabel + ". Frame as an observation or question, not a prescription. Example tone: 'In most " + ctx.vertLabel.toLowerCase() + " companies we see, the business case never reaches the implementation team because...' NOT 'You should build a handoff template.'\" },\n    { \"area\": \"short label\", \"observation\": \"2-3 sentences. A different area of the architecture that's worth examining. Be specific to " + ctx.vertLabel + " and their scores.\" },\n    { \"area\": \"short label\", \"observation\": \"2-3 sentences. A third area. This one should connect to the structural reason proof is hard in this vertical (from 'why proof is hard' above).\" }\n  ],\n  \"roleSpecificInsight\": \"2-3 sentences for a " + ctx.role + ". How should they think about this \u2014 not as a CS problem but as a commercial architecture issue? Who else needs to be involved? Keep it practical, not political.\"\n}";
