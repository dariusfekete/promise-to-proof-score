...
var VERTICALS = [
  ...
  { id: "fintech", label: "FinTech / Banking SaaS",
    valuePromise: "improved risk management, regulatory compliance, operational efficiency, reduced fraud",
    typicalOutcomes: "20-40% improvement in processing efficiency, measurable reduction in fraud losses, compliance cost reduction, faster onboarding",
    proofChallenge: "Banking software operates where firm economics, monetary policy, and regulatory pressure all intersect. Portfolio performance is inseparable from credit cycles. Fraud prevention is invisible. Compliance is mandated, making ROI framing awkward.",
    renewalEvidence: "processing efficiency trends, fraud detection rates with false-positive adjustment, compliance cost per regulatory cycle, portfolio quality metrics with cycle normalization",
    whyProofIsHard: "Three problems at once. First, portfolio performance moves with interest rates and credit cycles \u2014 your platform's contribution is tangled with macro conditions nobody controls. Second, your best value is fraud you prevented and compliance you maintained, neither of which shows up as a line item. Third, some of your value accrues to the customer's customers or to the financial system itself \u2014 real value that no private ROI model captures.",
    proofShape: "The noisiest environment combined with invisible prevention value. You're proving your worth in a world where the biggest forces affecting outcomes have nothing to do with your software.",
    whatMakesItWorse: "Break 4 is the structural crux \u2014 credit cycle noise makes portfolio attribution impossible, and prevention value is invisible. Break 3 is hard because meaningful metrics require cycle-normalization that most CS teams can't perform. Break 1 matters because Sales often promises efficiency gains (measurable) when the real value is risk reduction (much harder to measure).",
    whatDoesntWork: "Portfolio performance attribution without cycle-normalization \u2014 a rising market makes everyone look good. Ignoring fraud prevention value because it's hard to measure. Presenting compliance value as optional ROI \u2014 it's mandated cost that your platform reduces, not optional upside."
  },
  { id: "proptech", label: "PropTech / Real Estate",
    valuePromise: "improved asset performance, reduced operational costs, better tenant outcomes, portfolio optimization",
    typicalOutcomes: "10-20% operating cost reduction, 15-30% faster leasing, 5-15% NOI improvement, maintenance cost reduction",
    proofChallenge: "Every property is physically unique, making benchmarking inherently approximate. Interest rate sensitivity makes identical software appear transformative or marginal depending on the macro environment.",
    renewalEvidence: "NOI trend by property with market normalization, operating cost per sqft trajectory, occupancy and leasing velocity, maintenance cost reduction with asset-age adjustment",
    whyProofIsHard: "Every building is different. Two identical platforms produce different results on different properties because the properties themselves are different \u2014 age, location, tenant mix, physical condition. On top of that, real estate is the most interest-rate-sensitive industry: the same software looks indispensable at 2% rates and marginal at 5.5%. And asset optimization plays out over decades, not quarters.",
    proofShape: "Value is real but non-comparable across assets, confounded by macro cycles, and takes years to compound. You can't benchmark property A against property B, and you can't separate the platform's contribution from the rate environment.",
    whatMakesItWorse: "Break 4 is hard because cross-property comparison is unreliable. Break 3 is hard because the right metrics require physical-world data collection (sensors, building systems). Break 1 matters because business cases built on portfolio-level benchmarks often don't hold at the individual property level.",
    whatDoesntWork: "Cross-property benchmarking without heavy contextualization. Simple before/after without rate-environment adjustment. Presenting portfolio averages when every asset has unique characteristics."
  },
  { id: "procurement", label: "Procurement / Source-to-Pay",
    valuePromise: "cost savings, supplier risk reduction, compliance improvement, strategic sourcing enablement",
    typicalOutcomes: "5-15% addressable spend savings, 30-50% faster sourcing cycles, measurable compliance improvement",
    proofChallenge: "Procurement has marketed against a savings metric for decades, creating a ceiling on perceived value. As procurement matures, remaining savings shrink, creating a perception of declining platform value. The real strategic value \u2014 compliance, risk avoidance \u2014 is invisible.",
    renewalEvidence: "savings trend with category-mix adjustment, compliance rate trajectory, supplier risk incident data, sourcing cycle time, strategic value metrics beyond savings",
    whyProofIsHard: "You're trapped by your own success metric. Procurement has sold on 'savings' for so long that the metric has a structural ceiling \u2014 as the easy savings get captured, remaining opportunities shrink, and the platform appears to deliver less value each year. Meanwhile, the real value has shifted to compliance (the 200 maverick purchases nobody will ever see that you prevented) and risk reduction (the supplier disruption that didn't happen). But nobody measures those because the conversation is still anchored on savings.",
    proofShape: "The visible metric has a ceiling that makes value appear to decline over time, even as the invisible strategic value increases. The proof problem isn't measurement difficulty \u2014 it's measuring the wrong thing.",
    whatMakesItWorse: "Break 4 is hard because strategic value can't be measured by the savings metric. Break 3 is the structural bottleneck \u2014 CS measures savings because that's what the business case promised, but the real value has moved elsewhere. Break 1 matters because Sales locks in savings expectations that trap every future renewal conversation.",
    whatDoesntWork: "Doubling down on savings tracking as the primary proof strategy \u2014 it reinforces the ceiling. Ignoring that procurement decisions are decentralized \u2014 value gets realized by people who don't think of themselves as procurement users. Treating compliance and risk avoidance as secondary to savings in the value narrative."
  },
  { id: "clinical", label: "Clinical / Life Sciences",
    valuePromise: "faster trial execution, regulatory compliance, improved data quality, reduced submission risk",
    typicalOutcomes: "20-40% reduction in trial startup time, 30-50% improvement in data quality metrics, reduced regulatory risk",
    proofChallenge: "Drug development takes 10-15 years. The feedback loop between software investment and commercial outcome is the longest in enterprise SaaS. Trial outcomes are binary: the drug passes or fails. GxP validation creates legally mandated switching costs.",
    renewalEvidence: "trial execution efficiency trends, data quality metrics, regulatory submission success rates, re-validation avoidance value",
    whyProofIsHard: "The ultimate outcome takes a decade to materialize. A system purchased in 2020 might contribute to a drug approval in 2032 and revenue in 2035. In between, the only evidence is process efficiency \u2014 faster startup, cleaner data, fewer regulatory queries. And the final outcome is violently binary: the drug either passes or it doesn't. Attributing a continuous software contribution to a binary outcome that happens 10 years later is mathematically insoluble at the individual trial level.",
    proofShape: "The most extreme combination of delayed outcomes and pass/fail results in enterprise software. You can prove you made the process better. You cannot prove you made the drug succeed.",
    whatMakesItWorse: "Break 4 is structurally the hardest in all of B2B SaaS for clinical \u2014 binary outcome plus 10-15 year lag. Break 3 is hard because meaningful outcome data may not exist yet. Break 2 is critical because GxP validation means early configuration decisions have consequences that last decades.",
    whatDoesntWork: "Trying to prove ROI through trial outcomes \u2014 the timeline makes it impossible for most contracts. Ignoring that GxP validation is a legally mandated switching cost that functions as retention independent of outcome proof. Measuring individual trial success as a platform metric \u2014 too many variables, binary outcome, sample size of one."
  },
  { id: "legal", label: "Legal Technology",
    valuePromise: "improved matter efficiency, reduced risk, better outcomes, knowledge management",
    typicalOutcomes: "20-40% reduction in matter handling time, measurable risk score improvement, improved contract quality",
    proofChallenge: "Every legal matter has unique facts, making case-by-case benchmarking unreliable. Outcomes are often binary (verdict, ruling) and the software's contribution is inseparable from the lawyer's expertise.",
    renewalEvidence: "matter efficiency trends, risk metrics, knowledge reuse rates, time-to-completion by matter type",
    whyProofIsHard: "Two things make legal tech proof uniquely hard. First, every matter is different \u2014 different facts, different jurisdiction, different opposing counsel. You can't benchmark Matter A against Matter B because they're fundamentally not comparable. Second, the software's value is tangled with the lawyer's judgment. The better the lawyer, the less credit the platform gets. A brilliant brief assisted by your AI looks like a brilliant lawyer, not brilliant software.",
    proofShape: "Value is real but inseparable from human expertise and impossible to generalize. Each success story has unique facts that limit its applicability to other matters.",
    whatMakesItWorse: "Break 4 is the crux \u2014 binary outcomes plus unique facts plus expert entanglement make clean attribution nearly impossible. Break 5 is hard because case studies have minimal generalizability across different matter types. Break 3 is moderately hard because process efficiency is measurable even when outcome attribution is not.",
    whatDoesntWork: "Outcome-level attribution for individual matters \u2014 too many unique variables. Cross-matter benchmarking without controlling for complexity and facts. Ignoring that legal precedent changes retroactively \u2014 what was good practice last year may not be this year."
  },
...
// ── PATTERN ARCHETYPES ───────────────────────────────
function detectPattern(bs) {
  var front = (bs[0] + bs[1]) / 2;
  var mid = bs[2];
  var back = (bs[3] + bs[4]) / 2;
  var spread = Math.max.apply(null, bs) - Math.min.apply(null, bs);
  if (spread <= 4) return { id: "uniform", name: "Uniform gap", desc: "All five links are similarly weak. This isn't one broken handoff or one missing metric. The commercial architecture was never built to connect promises to proof. The good news: there's no single bottleneck blocking progress. The harder truth: this requires a systemic redesign, not a point fix." };
  if (front >= 12 && back <= 7) return { id: "front_heavy", name: "Strong front, weak back", desc: "Your Sales-to-Implementation handoff works, but the evidence chain collapses after go-live. The business case gets built and delivered, but nobody measures whether the promise was kept. This is the most common pattern: companies invest in selling and delivering, but not in proving." };
  if (front <= 7 && back >= 12) return { id: "back_heavy", name: "Weak front, strong back", desc: "When you do have outcome data, you use it well. But the front of the chain is broken: the business case either doesn't exist or doesn't survive the handoff to implementation. You're good at proving value when you have the data, but the data isn't being generated consistently because the promise was never structured for measurement." };
  if (front >= 10 && mid <= 5 && back >= 8) return { id: "broken_middle", name: "Broken middle", desc: "The promise gets made and the feedback loop works, but the measurement layer in the middle is missing. Your team tracks activity (logins, usage, health scores) instead of outcomes. This is the 'activity trap': everything looks green on the dashboard while the customer's CFO is quietly asking what they got for their money." };
  if (bs[4] <= 5 && front >= 10) return { id: "open_loop", name: "Open loop", desc: "Everything works except the feedback loop. Proven outcomes never reach Sales. Every new deal starts from theoretical ROI instead of building on verified results from similar customers. Your best proof points are invisible to the people who need them most. This is the easiest pattern to fix and the most commonly ignored." };
  if (bs[0] <= 5) return { id: "broken_start", name: "Promise problem", desc: "The chain breaks at the very beginning. The business case that closes deals either doesn't exist in a structured form or doesn't survive past signature. Everything downstream suffers because implementation, CS, and renewal conversations are all flying blind on what was actually promised." };
  return { id: "mixed", name: "Mixed pattern", desc: "Your chain has specific strong and weak points rather than a single clean pattern. The breaks aren't uniform, which means targeted improvements at the weakest links can have outsized impact." };
}

// ── BREAK CASCADES ───────────────────────────────────
function getCascades(bs) {
  var cascades = [];
  if (bs[0] <= 5 && bs[2] <= 10) cascades.push("Your weak handoff (Promise > Deliver) is making measurement harder downstream. CS can't track outcomes against the business case because they never received it. Fixing measurement without fixing the handoff means CS is guessing at what to measure.");
  if (bs[1] <= 5 && bs[2] <= 10) cascades.push("The broken go-live handoff (Deliver > Operate) starves your measurement system. Without outcome targets set at go-live, CS defaults to tracking product activity because there's nothing else to anchor on. The measurement gap isn't a CS failure. It's an information gap CS inherited.");
  if (bs[2] <= 5 && bs[3] <= 10) cascades.push("You can't prove what you didn't measure. The weak Operate > Measure link makes attribution (Measure > Prove) structurally impossible. No amount of renewal preparation fixes this because the underlying data was never collected.");
  if (bs[3] <= 5 && bs[4] <= 10) cascades.push("Without proof, the feedback loop has nothing to feed back. Sales builds the next business case from theory instead of evidence, which means the next customer's promise is also unverifiable. The chain degrades over time.");
  if (bs[4] <= 5 && bs[0] <= 10) cascades.push("The broken feedback loop (Prove > Decide) is weakening your promise over time. Sales doesn't have verified outcomes to build business cases from, so each new deal relies on theoretical ROI that's harder to defend against skeptical buyers.");
  return cascades;
}

// ── CUSTOMER-SIDE PERSPECTIVE ────────────────────────
var CUSTOMER_VIEW = [
  "Your customer's champion walks into a budget review. Their CFO asks: 'We spent $X on this platform. What did we get?' The champion opens your last QBR deck: adoption metrics, health scores, feature usage. The CFO asks again: 'But what did we get?' The champion doesn't have an answer, because the business case that justified the purchase was never connected to what happened after go-live. The renewal becomes a price conversation.",
  "Your customer went live three months ago. The implementation team handed over a working system. Nobody told the customer what outcomes to expect or when. Six months later, the product is being used, but nobody on the customer side can say whether it's working as intended, because 'working as intended' was never defined in business terms. When the renewal comes up, the default question is 'do we still need this?' instead of 'should we expand this?'",
  "Your customer's CS team sends a monthly report: logins up, feature adoption healthy, NPS at 42. The customer's operations leader nods politely but doesn't forward it to their CFO, because none of those metrics connect to the cost reduction or efficiency gain that was promised during the sale. The report answers 'are people using it?' but not 'is it working?' Those are different questions with very different implications for renewal.",
  "Your customer achieved real results from your platform, but the evidence is scattered across their ERP, their finance system, and three spreadsheets that a project manager maintained for a few months before moving on. When renewal arrives, neither your team nor theirs can assemble the proof. The customer knows the platform works. They just can't prove it to their CFO. So the conversation defaults to 'can we get a better price?' instead of 'should we expand?'",
  "Your Sales team just closed a deal using a business case built from theoretical ROI benchmarks. The customer's procurement team asked: 'Can you show us results from a similar company?' Your team shared a case study from two years ago with vague metrics because the detailed outcome data from recent customers was never captured. The prospect signed anyway, but at a lower price point because the proof wasn't compelling. Meanwhile, three of your current customers delivered results that would have closed the deal at full price, but nobody inside your company knows.",
];
...
export default function App(){
  ...
  var pattern=detectPattern(bscores);
  var cascades=getCascades(bscores);
  var customerView=CUSTOMER_VIEW[weakest]||CUSTOMER_VIEW[0];
  ...
  var handleGate=async function(){
    ...
    setLoading(true);
    var result=await getInsights({role:role,size:size,vertLabel:vert?vert.label:"B2B SaaS",vd:vert||VERTICALS[12],total:total,bs:bscores,nrr:nrr,bench:nrrBench,weak:weakest,patternName:pattern.name});
    setInsights(result);setLoading(false);
  };

  var handleDL=function(){
    var doc=generateReport({name:name,email:email,vertical:vert?vert.label:"",size:size,role:role,nrr:nrr,totalScore:total,breakScores:bscores,weakestBreak:weakest,insights:insights,verticalData:vert||VERTICALS[12],nrrBench:nrrBench,pattern:pattern,cascades:cascades,customerView:customerView});
  ...
  return(<><style>{S}</style><div className="wrap" ref={topRef}>
    ...
    {/* Pattern archetype */}
    <div className="rbox purple fade"><div className="rbox-label" style={{color:"var(--accent2)"}}>Your pattern: {pattern.name}</div><div className="rbox-text">{pattern.desc}</div></div>

    {/* Customer perspective */}
    <div className="rbox fade"><div className="rbox-label" style={{color:"var(--fg3)"}}>What your customer experiences</div><div className="rbox-text">{customerView}</div></div>
    ...
    {/* Break cascades */}
    {cascades.length>0&&<div className="rbox amber fade"><div className="rbox-label" style={{color:"#F59E0B"}}>How your breaks compound</div><div className="rbox-text">{cascades.map(function(c,i){return <span key={i}>{i>0&&<><br/><br/></>}{c}</span>;})}</div></div>}
